Wouldn't install... 

This thing dissolves file systems into their components. 

Like your PhD Adviser finding his car's been disassembled. 


Could easily be made into a copy all elements from a structure like it's a fire sale, but this one empties them and pops the empty directories like ...  well see my upgrade to class structure file to coming. 


So.. catastrophic recovery from an AI, that left some files but nonsense structure?  I don't even remember the program that did it.. something like adobe making a collage duplicate, but something happened to my other files was a possiblity, not the first time I assumed a commercial program worked, so often should prefere the old console text interface...  professional "Idle" time to play with pythone a litte bit, in case there is someone hiring those of us vastly overqualified for the run of the mill system admin or programmer, but haven't an envied position some otherwise clueless HR/Exec looking to steal from some other business, not being STEM but MBA.. but if you're an exec and competent, let's talk. 

On the most of the global, at least from a U.S. perspective sounds like letting AI take over yours system and oops if the deveopment goes as it's all connected?  https://www.linkedin.com/posts/shanemaddoxbruce_well-some-of-us-dont-need-an-ai-to-do-that-share-7455077179685687297-GxTq

And old school as some recent disconnected backups besides the division of production and development, ...  Today, it's the AI controlled rollback they build their point and click apps on, but could happen if they were at all doing the job with due diligence, Statics were DAT tapes in the day, few different versions going back week, months if not exhaustively used and tested with teams of let's look at every link, test every option QA, or any changes in underlying data structures, etc and so.. I real 'big iron' man that keeps the servers will eventually supposed to have a room of those.. (mine were in a series of three fires, which they aren't allowed to talk about but..)  .. but those jobs were automated away...  Again, Just saying.  If they don't appreciate the toy makers, take away their toys. 
